...
data2 = data.merge(prd, how='inner', on='product_id').merge(users, how='inner', on='user_id')
...
val=bst.predict(d_val)
accuracy=sum((val>0.5).astype(int)==y_val)/y_val.shape[0]
# In[ ]:


d_test = xgboost.DMatrix(X_test.drop(['eval_set', 'user_id', 'order_id', 'reordered', 'product_id'], axis=1))
X_test.loc[:,'reordered'] = (bst.predict(d_test) > 0.5).astype(int)
...
sm=sample_submission[['order_id']].merge(X_test[X_test.reordered==1],how='left').fillna('None')
sm=sm[['order_id','user_id','product_id']]
sm.to_csv('sm.csv',index=False)
submit_final = sample_submission[['order_id']].merge(submit, how='left').fillna('None')
sub=submit_final.merge(right=orders[['order_id', 'user_id']], how='left', on='order_id')
sub.to_csv("python_test.csv", index=False)
